...
PhongBSDF::PhongBSDF(PrivatePtr<opg::Scene> _scene, const opg::Properties &_props) :
    BSDF(std::move(_scene), _props)
{
    m_bsdf_data.diffuse_color = glm::vec3(_props.getVector("diffuse_color", glm::vec4(0)));
    m_bsdf_data.specular_F0 = glm::vec3(_props.getVector("specular_f0", glm::vec4(0)));
    m_bsdf_data.exponent = _props.getFloat("exponent", 1);
    m_bsdf_data_buffer.alloc(1);
    m_bsdf_data_buffer.upload(&m_bsdf_data);
}
...
void PhongBSDF::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    auto ptx_filename = opg::getPtxFilename(OPG_TARGET_NAME, "bsdfmodels.cu");
    OptixProgramGroup eval_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__phong_evalBSDF" });
    OptixProgramGroup sample_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__phong_ward_ggx_sampleBSDF" });

    uint32_t eval_bsdf_index = sbt->addCallableEntry(eval_bsdf_prog_group, m_bsdf_data_buffer.data());
    uint32_t sample_bsdf_index = sbt->addCallableEntry(sample_bsdf_prog_group, m_bsdf_data_buffer.data());

    BSDFVPtrTable vptr_table_data;
    vptr_table_data.component_flags = BSDFComponentFlag::DiffuseReflection | BSDFComponentFlag::GlossyReflection;
    vptr_table_data.evalCallIndex = eval_bsdf_index;
    vptr_table_data.sampleCallIndex = sample_bsdf_index;
    m_vptr_table.allocIfRequired(1);
    m_vptr_table.upload(&vptr_table_data);
...
WardBSDF::WardBSDF(PrivatePtr<opg::Scene> _scene, const opg::Properties &_props) :
    BSDF(std::move(_scene), _props)
{
    m_bsdf_data.diffuse_color = glm::vec3(_props.getVector("diffuse_color", glm::vec4(0)));
    m_bsdf_data.specular_F0 = glm::vec3(_props.getVector("specular_f0", glm::vec4(0)));
    float isotropic_roughenss = _props.getFloat("roughness", 1);
    m_bsdf_data.roughness_tangent = _props.getFloat("roughness_tangent", isotropic_roughenss);
    m_bsdf_data.roughness_bitangent = _props.getFloat("roughness_bitangent", isotropic_roughenss);
    m_bsdf_data_buffer.alloc(1);
    m_bsdf_data_buffer.upload(&m_bsdf_data);
}
...
void WardBSDF::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    auto ptx_filename = opg::getPtxFilename(OPG_TARGET_NAME, "bsdfmodels.cu");
    OptixProgramGroup eval_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__ward_evalBSDF" });
    OptixProgramGroup sample_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__phong_ward_ggx_sampleBSDF" });
...
GGXBSDF::GGXBSDF(PrivatePtr<opg::Scene> _scene, const opg::Properties &_props) :
    BSDF(std::move(_scene), _props)
{
    m_bsdf_data.diffuse_color = glm::vec3(_props.getVector("diffuse_color", glm::vec4(0)));
    m_bsdf_data.specular_F0 = glm::vec3(_props.getVector("specular_f0", glm::vec4(0)));
...
void GGXBSDF::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    auto ptx_filename = opg::getPtxFilename(OPG_TARGET_NAME, "bsdfmodels.cu");
    OptixProgramGroup eval_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__ggx_evalBSDF" });
    OptixProgramGroup sample_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__phong_ward_ggx_sampleBSDF" });
...
OPG_REGISTER_SCENE_COMPONENT_FACTORY(PhongBSDF, "bsdf.phong");
OPG_REGISTER_SCENE_COMPONENT_FACTORY(WardBSDF, "bsdf.ward");
OPG_REGISTER_SCENE_COMPONENT_FACTORY(GGXBSDF, "bsdf.ggx");
