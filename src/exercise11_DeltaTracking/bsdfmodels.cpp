    ...
    BSDF(std::move(_scene), _props)
{
    ...
    m_bsdf_data.roughness = _props.getFloat("roughness", 1);
    m_bsdf_data_buffer.alloc(1);
    m_bsdf_data_buffer.upload(&m_bsdf_data);
}
...
void GGXBSDF::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    ...
    OptixProgramGroup sample_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__ggx_sampleBSDF" });
...
NullBSDF::NullBSDF(PrivatePtr<opg::Scene> _scene, const opg::Properties &_props) :
    BSDF(std::move(_scene), _props)
{
}
...
void NullBSDF::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    auto ptx_filename = opg::getPtxFilename(OPG_TARGET_NAME, "bsdfmodels.cu");
    OptixProgramGroup eval_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__nullbsdf_evalBSDF" });
    OptixProgramGroup sample_bsdf_prog_group = pipeline->addCallableShader({ ptx_filename, "__direct_callable__nullbsdf_sampleBSDF" });

    uint32_t eval_bsdf_index = sbt->addCallableEntry(eval_bsdf_prog_group, nullptr);
    uint32_t sample_bsdf_index = sbt->addCallableEntry(sample_bsdf_prog_group, nullptr);

    BSDFVPtrTable vptr_table_data;
    vptr_table_data.component_flags = +BSDFComponentFlag::IdealTransmission;
    vptr_table_data.evalCallIndex = eval_bsdf_index;
    vptr_table_data.sampleCallIndex = sample_bsdf_index;
    m_vptr_table.allocIfRequired(1);
    m_vptr_table.upload(&vptr_table_data);
...
OPG_REGISTER_SCENE_COMPONENT_FACTORY(NullBSDF, "bsdf.null");
