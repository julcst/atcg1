#include "volumepathtracingraygenerator.h"
...
VolumePathtracingRayGenerator::VolumePathtracingRayGenerator(PrivatePtr<opg::Scene> _scene, const opg::Properties &_props) :
    ...
    m_camera_revision(~0),
    m_subframe_index(0),
    m_initial_camera_medium{ _props.getComponentAs<opg::Medium>("initial_camera_medium") }
{
    m_launch_params_buffer.alloc(1);
}

VolumePathtracingRayGenerator::~VolumePathtracingRayGenerator()
{
}

void VolumePathtracingRayGenerator::initializePipeline(opg::RayTracingPipeline *pipeline, opg::ShaderBindingTable *sbt)
{
    std::string ptx_filename = opg::getPtxFilename(OPG_TARGET_NAME, "volumepathtracingraygenerator.cu");
...
void VolumePathtracingRayGenerator::finalize()
{
...
void VolumePathtracingRayGenerator::launchFrame(CUstream stream, const opg::TensorView<glm::vec3, 2> &output_buffer)
{
    ...
    VolumePathtracingLaunchParams launch_params;
    ...
    launch_params.initial_camera_medium = m_initial_camera_medium != nullptr ? m_initial_camera_medium->getMediumVPtrTable() : nullptr;
...
OPG_REGISTER_SCENE_COMPONENT_FACTORY(VolumePathtracingRayGenerator, "raygen.volumepathtracing");
